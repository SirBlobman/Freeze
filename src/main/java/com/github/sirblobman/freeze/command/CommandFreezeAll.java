...
public class CommandFreezeAll implements Command<CommandSourceStack> {
    ...
    public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (count <= 0) {
            Component component = messages.getMessageComponent("freeze-all-failure");
            if (component == null) {
                component = Component.text("No players were frozen.");
            }
        ...
        messages.sendMessage(sender, "freeze-all", replacer);
    ...
    private boolean freeze(@NotNull Player player) {
        ...
        freezeManager.freeze(player, expireTime);
        return true;
    }
}
